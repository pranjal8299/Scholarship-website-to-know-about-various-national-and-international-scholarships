...
function matchAndDisplay(data, searchState) {
    ...
    for (let scholarship of data) {
        if (!searchState || (scholarship.state && scholarship.state.toLowerCase() === searchState.toLowerCase())) {
    ...
    if (matchedElements.length > 0) {
        ...
        noResultMessage.textContent = 'There are no scholarships available for the entered state.';
        linksContainer.appendChild(noResultMessage);
    }
}

function refresh() {
    let searchState = document.getElementById('searchInput').value.trim();
    matchAndDisplay(scholarships, searchState);
}
